...
public class ShareHomo extends JFrame implements ActionListener {
 ...
 public ShareHomo() {
  ...
  menu.setBorder(BorderFactory.createTitledBorder("Menu Options:"));
  menu.setPreferredSize(new Dimension(200, 220));
  JPanel originalPan = new JPanel();
  originalPan.setBorder(BorderFactory.createTitledBorder("Secret Numbers:"));
 ...
 public String SelectFile() {
  ...
  if (buttonClicked == JFileChooser.APPROVE_OPTION) {
   path = "" + jfc.getCurrentDirectory() + "\\" + (jfc.getSelectedFile().getName()).toLowerCase();
  }
  return path;
 }
 ...
 public void actionPerformed(ActionEvent aE) {
  if (aE.getSource() == bttLoad) {
   txtRead(SelectFile());
  }
  if (aE.getSource() == bttCreate) {
   txtShare();
  }
  if (aE.getSource() == bttRecon) {
   txtRecon();
  }
  if (aE.getSource() == bttAveg) {
   txtAveg();
  }
  if (aE.getSource() == bttDisplay) {
   JOptionPane.showMessageDialog(null, instructTxt);
  }
  if (aE.getSource() == bttAbout) {
   JOptionPane.showMessageDialog(null, aboutTxt);
  }
  if (aE.getSource() == bttQuit) {
   System.exit(0);
  }
 }
